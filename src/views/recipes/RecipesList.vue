...
<section class="content">
        <div class="container">
            <h1 class="title">Daftar Resep Masakan</h1>
    ...
    computed: {
        loadRecipes() {
            if(this.$store.getters.getToken !== '') {
                fetch(`http://127.0.0.1:8000/api/recipes?search=${this.searchValue}&recipename=${this.recipename}&categoryname=${this.categoryname}&time=${this.time}`, {
                    headers: {
                    ...
                    const recipes = [];
                        for(const key in data.data) {
                            const recipe = {
                                ...
                                userIdFav: data.data[key].userIdFav,
...
<style scoped>
@import '../../assets/css/recipes/recipesList.css';
</style>